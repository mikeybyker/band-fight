...
const getImages = (image, specified = null) => {
...
// Slightly hacked version follows...
...
export class Artist {
  ...
  constructor(
    public image: any = [],            // Gets mutated by getImages
    public mbid: string = '',
    public id: string = '',            // Copy of mbid
    public name: string = '',
    // Not interested in any of the following info sent by last.fm
    // public bio: any = {},    
    // public listeners: string = '',
...
export class LastFM {
  ...
  constructor( @Inject('LastFMConfig') private config: LastFMConfig, public http: Http) {
    ...
    const setup: any = { format: config.format };
  ...
  */
  ...
  private _http(settings: LastFMOptions = {}, options: LastFMOptions = {}): Observable<any> {
    ...
    return this.http.get(this.config.endPoint, { search: params })
      // return this.http.get('http://localhost:3000/api', { search: params })
      // return this.http.get('https://server-prxkebeupz.now.sh/api', { search: params })
      .map(res => res.json())
